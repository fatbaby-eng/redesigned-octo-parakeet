(function () {
  ...
  // ============================================================
  // Per-project report (markdown + printable HTML)
  // ============================================================
  ...
  function generateReportMarkdown(p, log, refs) {
    var lines = [];
    lines.push("# " + (p.title || "Untitled"));
    lines.push("");
    var meta = [];
    meta.push("**Area:** " + (p.area || "personal"));
    meta.push("**Status:** " + (p.status || "active"));
    if (p.pinned) meta.push("**Pinned**");
    if (p.updatedAt) meta.push("**Updated:** " + fmtReportDate(p.updatedAt));
    if (p.createdAt) meta.push("**Created:** " + fmtReportDate(p.createdAt));
    lines.push(meta.join("  \n"));
    lines.push("");
    if (p.liveUrl) { lines.push("**Live URL:** [" + p.liveUrl + "](" + p.liveUrl + ")"); lines.push(""); }
    if (p.primaryPath) { lines.push("**Primary path:** `" + p.primaryPath + "`"); lines.push(""); }

    if (p.nextAction) {
      lines.push("## Active Quest");
      lines.push("");
    ...
    if (p.dossier) {
      lines.push("## Architecture & Origin");
      lines.push("");
      lines.push(p.dossier);
      lines.push("");
    }
    if ((p.stack || []).length) {
      lines.push("## Stack");
      lines.push("");
      lines.push(p.stack.map(function (s) { return "`" + s + "`"; }).join(" · "));
      lines.push("");
    }
    if ((p.tags || []).length) {
      lines.push("## Tags");
    ...
    if ((p.docPaths || []).length) {
      lines.push("## Docs");
      lines.push("");
      p.docPaths.forEach(function (d) { lines.push("- `" + d + "`"); });
      lines.push("");
    }
    if ((p.related || []).length) {
      ...
      p.related.forEach(function (rid) {
        var rel = state.projects.find(function (x) { return x.id === rid; });
        lines.push("- " + (rel ? rel.title + " (`" + rid + "`)" : "`" + rid + "`"));
      });
      lines.push("");
    }
    if ((log || []).length) {
      ...
      log.forEach(function (e) {
        lines.push("- **" + fmtReportDate(e.at) + "** — " + (e.text || ""));
      });
      lines.push("");
    }
    if ((refs || []).length) {
      ...
      refs.forEach(function (r) {
        lines.push("### " + fmtReportDate(r.at) + " — " + (r.question || "(no question)"));
        lines.push("");
        lines.push(r.answer || "");
        lines.push("");
      });
    ...
    lines.push("---");
    lines.push("");
    lines.push("*Generated by Atlas on " + new Date().toISOString().slice(0, 19).replace("T", " ") + " UTC*");
    return lines.join("\n");
  }

  // Lightweight markdown -> HTML renderer (good enough for reports).
  // Handles: headings, bold, italic, code spans, code fences, links, lists, paragraphs.
  function mdToHtml(md) {
    if (!md) return "";
    var i, lines = md.split(/\r?\n/), out = [], inUl = false, inCode = false, codeBuf = [];
    ...
    function inline(s) {
      ...
      // italic (avoid clashing with bold leftovers)
  ...
  function openReport() {
    ...
    if (!p) return;
    var md = generateReportMarkdown(p, state.currentLog, state.currentReflections);
  ...
  function wire() {
    ...
    document.getElementById("btn-report-modal").addEventListener("click", openReport);
    document.getElementById("btn-report-copy").addEventListener("click", function () {
      copyToClipboard(state.lastReportMd || "");
    });
    document.getElementById("btn-report-print").addEventListener("click", function () {
      window.print();
    });
    document.getElementById("btn-report-close").addEventListener("click", function () {
      document.getElementById("report-modal").close();
    });
    document.getElementById("report-modal").addEventListener("cancel", function (e) {
      e.preventDefault();
      document.getElementById("report-modal").close();
    });
